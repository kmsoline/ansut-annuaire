...
const ALLOWED = [
  "name", "category", "pays", "zone", "vibe", "description",
  "tags", "note", "price_range", "img", "link",
  "active", "sort_order",
] as const;
...
export async function PATCH(request: NextRequest, { params }: P) {
  ...
  try {
    const item = await dbUpdate("afrinomade_bons_plans", `id=eq.${id}`, payload);
    return NextResponse.json(item);
  } catch (e) {
    console.error("[bons-plans PATCH]", e);
...
export async function DELETE(_: NextRequest, { params }: P) {
  ...
  if (!isUUID(id)) return NextResponse.json({ error: "ID invalide" }, { status: 400 });
  try {
    await dbDelete("afrinomade_bons_plans", `id=eq.${id}`);
    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error("[bons-plans DELETE]", e);
